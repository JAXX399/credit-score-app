...
# --- THE SMART PART: TRAIN MODEL ON STARTUP ---
@st.cache_resource
def get_trained_model():
    # 1. Load Data
    # Try multiple paths to find the file (Local vs Cloud)
    data_path = 'german_credit_data.csv'
    if not os.path.exists(data_path):
        data_path = 'data/german_credit_data.csv'
        
    if not os.path.exists(data_path):
        st.error("❌ Data file not found! Please upload 'german_credit_data.csv' to the root folder.")
    ...
    if target_col not in df.columns:
        # Fallback for different data versions
    ...
    model_columns = list(X_encoded.columns) # Save this for alignment later
...
try:
    with st.spinner("Initializing AI Brain... (This runs once)"):
...
def show_home_page():
    ...
    if st.button("Calculate Credit Score", type="primary", use_container_width=True):
        if not MODEL_LOADED:
            st.error("Model could not be loaded. Please reload the page.")
        else:
